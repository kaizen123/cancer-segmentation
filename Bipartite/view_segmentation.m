function seg_im = view_segmentation(img,labels,out_path,only_name,display)
% display = 0;
% only_name = img_name;
% labels = label_img(:);
% add para_num and para_type after labels
% if para_type == 1
%     para_x = 'alpha';
% elseif para_type == 2
%     para_x = 'beta';
...
if Z > 1,    % makes image grey if it isn't already
...
% OutImgName_1 = fullfile(final_path, [only_name, '_', int2str(L),para_x,'_',num2str(para_num),'_out.bmp']);
...
label_img = reshape(labels,[X,Y]); [imgMasks,segOutline,imgMarkup,seg_im]=segoutput(disp_img,label_img,only_name);
...
% OutImgName_2 = fullfile(final_path, [only_name, '_', int2str(L),para_x,'_',num2str(para_num), '_out_c.bmp']);
...
%[imgMasks,segOutline,imgMarkup]=segoutput(disp_img,label_img,only_name);
if display == 1, figure; imshow(imgMarkup); end;
